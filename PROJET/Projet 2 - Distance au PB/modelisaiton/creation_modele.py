...
import mutualisation_traitement

warnings.filterwarnings('ignore')

eti31 = pd.read_excel("C:/Users/SPML0410/Documents/MachineLearning2/projet/eti31_traite.xlsx")
eti31 = eti31.drop(columns="iar_ndfictif", axis=1)
index_nan = eti31[eti31.isnull().any(axis=1)].index
...
def stat_modele(true, pred, modele, arrondi=3, affichage=False):
    ...
    texte = "Accuracy : proportion de predictions correctes (VP + VN /(VP+VN+FN+FP)) : " + str(accuracy) + "% \n" + \
            "Rappel : proportion de reussites correctement predites sur l'ensemble des reussites (VP/(VP+FN)): " + \
            str(rappel) + "% \n" + "Specificite : proportion d'echecs correctement predits sur l'ensemble des " \
                                   "echecs (VN/(VN+FP)) : " + str(
        specificity) + "% \n" + "Valeur predictive negative: proportion de " \
                                "veritables echecs parmi les echecs predits (VN/(VN+FN)) : " + str(vpn) + "% \n"

    if affichage:
        print(texte)
    fichier = open("modele_stats/" + modele + ".txt", "a")
    fichier.truncate(0)
    fichier.write(texte)
    fichier.close()
...
def pred_seuil(df, modele_name, seuil=0.5):
    ...
    """
    mod = joblib.load(
        "C:/Users/SPML0410/Documents/MachineLearning2/projet/modelisaiton/modeles/" + modele_name + ".joblib")
    if modele_name == "svm":
...
def modele_arbre_decision():
    ...
    arbre = DecisionTreeClassifier(criterion="gini", min_samples_leaf=1, max_depth=3)
    arbre.fit(train_df_x, train_df_y)
    acc, rap, pre, spe, vpn = stat_modele(test_df_y, arbre.predict(test_df_x), "arbre_decision", affichage=False)
    joblib.dump(arbre, "modeles/arbre_decision.joblib")
    return spe, vpn
...
def modele_arbre_boosting():
    ...
    """
    train_df_x, train_df_y, test_df_x, test_df_y = creation_test_train(eti31, "Reussite")
    arbre = RandomForestClassifier(criterion="gini", min_samples_leaf=1, max_depth=3)
    arbre.fit(train_df_x, train_df_y)
    acc, rap, pre, spe, vpn = stat_modele(test_df_y, arbre.predict(test_df_x), "arbre_boosting", affichage=False)
    joblib.dump(arbre, "modeles/arbre_boosting.joblib")
    return spe, vpn
...
def modele_nn():
    ...
    """
    train_df_x, train_df_y, test_df_x, test_df_y = creation_test_train(eti31, "Reussite")
    neigh = KNeighborsClassifier(n_neighbors=3, weights="uniform", p=1)
    neigh.fit(train_df_x, train_df_y)
    acc, rap, pre, spe, vpn = stat_modele(test_df_y, neigh.predict(test_df_x), "nearest_neighbors", arrondi=5,
                                          affichage=False)
    joblib.dump(neigh, "modeles/nearest_neighbors.joblib")
    return spe, vpn
...
def modele_regression_logistique():
    ...
    acc, rap, pre, spe, vpn = stat_modele(test_df_y, lgr.predict(test_df_x), "regression_logistique", arrondi=5,
                                          affichage=False)
    joblib.dump(lgr,
                "modeles/logistic_regression.joblib")
    return spe, vpn
...
def modele_svm():
    ...
    acc, rap, pre, spe, vpn = stat_modele(test_df_y, clf.predict(test_df_x), "svm", arrondi=5, affichage=False)
    joblib.dump(clf, "modeles/svm.joblib")
    return spe, vpn
...
def modele_nb():
    ...
    acc, rap, pre, spe, vpn = stat_modele(test_df_y, mod.predict(test_df_x), "naive_bayes", arrondi=5, affichage=False)
    joblib.dump(mod, "modeles/naive_bayes.joblib")
    return spe, vpn
...
def resultats():
    ...
    for i in range(nombre_points):
        ...
        for modele_name in modele_type:
            match modele_name:
                case "arbre_decision":
                    modele = DecisionTreeClassifier(criterion="gini", min_samples_leaf=1, max_depth=5)
                case "arbre_boosting":
                    modele = RandomForestClassifier(criterion="gini", min_samples_leaf=1, max_depth=4)
                case "nearest_neighbors":
                    modele = KNeighborsClassifier(n_neighbors=3, weights="uniform", p=1)
...
def best_modeles():
    # Stats minimales
    stats_arbre_decision = [90, 80]
    stats_arbre_boosting = [83, 80]
    stats_nn = [95, 80]
    stats_reglog = [85, 60]
    stats_svm = [95, 95]
    stats_nb = [85, 75]
    ...
    for modele_name in modele_type:
        match modele_name:
            case "arbre_decision":
                current = [0, 0]
                stats = stats_arbre_decision
                while current[0] <= stats[0] or current[1] <= stats[1]:
                    current[0], current[1] = modele_arbre_decision()
            case "arbre_boosting":
                current = [0, 0]
                stats = stats_arbre_boosting
                while current[0] <= stats[0] or current[1] <= stats[1]:
                    current[0], current[1] = modele_arbre_boosting()

            case "nearest_neighbors":
                current = [0, 0]
                stats = stats_nn
                while current[0] <= stats[0] or current[1] <= stats[1]:
                    current[0], current[1] = modele_nn()
            case "regression_logisitque":
                current = [0, 0]
                stats = stats_reglog
                while current[0] <= stats[0] or current[1] <= stats[1]:
                    current[0], current[1] = modele_regression_logistique()

            case "svm":
                current = [0, 0]
                stats = stats_svm
                while current[0] <= stats[0] or current[1] <= stats[1]:
                    current[0], current[1] = modele_svm()
            case _:
                current = [0, 0]
                stats = stats_nb
                while current[0] <= stats[0] or current[1] <= stats[1]:
                    current[0], current[1] = modele_nb()
    return None
...
def prediction(type_modele, probas, seuil=0.5, chemin_fichier=None, chemin_dossier=None):
    ...
    dossier = mutualisation_traitement.complet(chemin_dossier=chemin_dossier,
                                               chemin_fichier=chemin_fichier)
    df = pd.read_excel(dossier + "/df_traite.xlsx")

    sauvegarde_no_designation = df["iar_ndfictif"]
    df = df.drop(columns="iar_ndfictif", axis=1)
    try:
        if probas:

            df["Proba échec"], df["Proba réussite"] = pred_probas(df, type_modele)
        else:
            df["Reussite"] = pred_seuil(df, modele_name=type_modele, seuil=seuil)
    ...
    df["iar_ndfictif"] = sauvegarde_no_designation
    df.to_excel(dossier + "\predictions_" + type_modele + ("_probas" if probas else "_seuil") + ".xlsx", index=False)
    return df


prediction("svm", False, seuil=0.9)
