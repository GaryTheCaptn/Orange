...
# On recupère le dataframe des scores
...
def traitement_entier(chemin_fichier=None, chemin_dossier=None, code_releve=False):
    """
    ...
    :param code_releve: (optionnel, default = None) Booléen indiquant si on prépare des données avec une labellisation
    ou non.
    ...
    if selection_reussie_f and selection_reussie_d:
        try:
            if not code_releve:
                ...
                vraies_colonnes = ["No DESIGNATION", "COMMENTAIRE DE SIG", "CODE RELEVE", "COMMENTAIRE", "BLOC NOTE",
                                   "COMMLITIGEPIDI"]
                dataframe = bonnes_colonnes(dataframe, vraies_colonnes)
            ...
            if not code_releve:
                ...
                dataframe = normalisation_for_all(dataframe, ["No DESIGNATION", "Reussite", "COMMENTAIRES"]).dropna(
                    axis=0)
